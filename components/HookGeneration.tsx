...
const HookGeneration = ({ videoId }: { videoId: string }) => {
    const { user } = useUser()
    ...
    return (
        <div className="rounded-lg flex flex-col gap-4 p-4 bg-white shadow-[0_1px_1px_rgba(0,0,0,0.05),0_4px_6px_rgba(34,42,53,0.04),0_24px_68px_rgba(47,48,55,0.05),0_2px_3px_rgba(0,0,0,0.04)]">
            <div className="min-w-52">
                <Usage
                    flag={FeatureFlag.GENERATE_HOOKS}
                    title="Video Hooks Generation"
                />
            </div>
            ...
            </div>
            {
                !titles?.length && <div className="text-center px-4 py-8 border-2 border-dashed border-gray-100 rounded-lg bg-gray-50">
                    <p className="text-muted-foreground font-cal"> No hook have been generated yet </p>
                    <p className="text-muted-foreground text-sm mt-1 ">Generate hooks to see them appear here</p>
